...
int run(void) {
	...
	while(state.keep_running) {
		...
		res =
		  pselect(state.max_fds, &read_fds, NULL, &except_fds, &timeout, NULL);
		if(res < 0) {
			perror("Select");
			discontinue();
			return 4;
...
int initialize(void) {
	...
	if((strcmp(state.output_path, "-") != 0)
	   && (strlen(state.output_path) > 0)) {
		if(state.verbosity > 4) {
...
int cleanup(void) {
	if(state.input >= 0) {
		if(state.verbosity > 5) {
			fprintf(stderr, "Closing input FD `%d`\n", state.input);
		}
		close(state.input);
	}
	if(state.output >= 0) {
		if(state.verbosity > 5) {
			fprintf(stderr, "Closing output FD `%d`\n", state.output
	...
	if(state.server >= 0) {
		if(state.verbosity > 5) {
			fprintf(stderr, "Closing server FD `%d`\n", state.server
...
int accept_client_in(void) {
	...
	if(client < 0) {
		...
		if(state.num_clients < state.max_clients) {
			...
			if(state.verbosity > 3) {
				fprintf(stderr, "Accepted client connection fd `%d`\n", client);
			}
			if(state.verbosity > 4) {
				fprintf(stderr,
				  "Currently serving `%d` clients (`%d` max)\n",
			...
			close(client);
			if(state.verbosity > 3) {
				fprintf(stderr, "Refused client connection fd `%d`\n", client);
			}
			client = -1;
		}
	}
...
int get_max_fd(void) {
	...
	for(fd = 0; fd < FD_SETSIZE; ++fd) {
		if(FD_ISSET(fd, &state.client_fds)) {
			if(state.verbosity > 6) {
				fprintf(stderr, "FD `%d` is a client connection FD\n", fd);
			}
			if(max < fd) {
				max = fd;
			}
...
const char* parse_string(int* arg,
  int argc,
  const char** argv,
  int opos,
  const char* sopt,
	...
	while(arg < argc) {
		...
		if(strcmp(tmp_str, "-h") == 0 || strcmp(tmp_str, "--help") == 0) {
			...
			break;
					  ...
					  parse_string(&arg, argc, argv, ofs + 1, "i", "input"))) {
			state.input_path = tmp_str;
		} else if((tmp_str =
					  parse_string(&arg, argc, argv, ofs + 2, "o", "output"))) {
			state.output_path = tmp_str;
		} else if((tmp_int = parse_num(&arg, argc, argv, ofs + 3, "p", "port"))
				  > 0) {
			state.port_no = tmp_int;
		} else if((tmp_int =
					  parse_num(&arg, argc, argv, ofs + 4, "m", "max-clients"))
				  > 0) {
			state.max_clients = tmp_int;
		} else if((tmp_int = parse_num(&arg, argc, argv, -1, "V", "verbosity"))
				  > 0) {
			...
			ofs++;
			fprintf(
			  stderr, "Incrementing verbosity to: `%d`\n", state.verbosity);
		} else {
...
const char* get_arg_value_str(int* arg,
  int argc,
  const char** argv,
  int opos,
  const char* stag,
...
const char* parse_arg_tag(int* arg,
  int argc,
  const char** argv,
  const char* stag,
  const char* ltag,
...
// do_parse_arg_tag
const char* do_parse_arg_tag(int* arg,
  int argc,
  const char** argv,
  int plen,
  const char* pfx,
  ...
  const char* vsep) {
	...
	if((tmp_str = do_parse_arg_tag(arg, argc, argv, 2, "--", ltag, vsep))) {
		return tmp_str;
				  ...
				  do_parse_arg_tag(arg, argc, argv, 1, "-", stag, vsep))) {
		return tmp_str;
